...
classdef Drone < handle
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function obj = Drone(axis,spaceDim,num_drones,pos_start)
            if nargin > 1
                obj.axis = axis;
                ...
                %obj.pos = [0;0;0];
                obj.pos = [pos_start(1);pos_start(2);pos_start(3)];
                
                %obj.pos_offset = [5.*(rand - 0.5),5.*(rand - 0.5),4];
                obj.pos_offset = [pos_start(1),pos_start(2),pos_start(3)];
                %obj.pos_offset = [5.*(pos_start(2) - 0.5),5.*(pos_start(3) - 0.5),4];
...
%             obj.axis.Title.String = ['Sim Time = ',num2str(obj.time,'%f'),' seconds'];
        ...
        function obj = change_pos_and_orientation(obj, X)
            t = obj.time;
            
            %vary position
            %pos_mat = [X(9),X(11),-X(7)] + obj.pos_offset;
            pos_mat = [X(9),X(11),X(7)];
        ...
        function update(obj, X)
            ...
            obj = change_pos_and_orientation(obj, X);
